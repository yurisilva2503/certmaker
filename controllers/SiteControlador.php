...
class SiteControlador extends Roteador
{
    ...
    public function confirmarConta(): void
    {
        ...
        $criar = $usuarioServico->cadastrarUsuario( $dadosUsuario['nome'], $dadosUsuario['email'], $dadosUsuario['telefone'], $dadosUsuario['senha'], $dadosUsuario['datanasc']); // método que insere os dados no DB
    ...
    public function cancelarConta(): void
    {
        ...
        // Marca como utilizado e define como cancelado
    ...
    public function admin(): void
{
    ...
    public function perfilAdminForm(): void
    {
        ...
        $imagemBase64 = $usuario->getImg(); // Mantém imagem atual se não enviar
    ...
    public function salvarModeloCertificado(): void
    {
        ...
        if ($verificacaoSessao) {
            ...
            if ($usuario['status'] && $usuario['usuario']->getPerfil() == 'admin') {
                ...
                // Processar elementos (mantendo compatibilidade com versão anterior)
                $elementos = $input['elementos'] ?? '[]';
                if (is_array($elementos)) {
                    $elementos = json_encode($elementos);
                }
                
                // Processar páginas - AGORA O BACKGROUND ESTÁ DENTRO DE CADA PÁGINA
                $paginas = $input['paginas'] ?? '[]';
                if (is_array($paginas)) {
                    $paginas = json_encode($paginas);
                }
                
                // Processar highResPreview se necessário
                ...
                try {
                    $resultado = $certificadoServico->criar(
                        ...
                        $elementos, 
                        $paginas,
                        null, // background agora é null pois está dentro das páginas
                    );
    ...
    public function salvarCertificado(): void
    {
        ...
        try {
            // Pega o corpo do request (JSON do fetch)
   ...
   public function obterModeloExistente(string $id): void
{
    ...
    try {
        ...
        // Verificar se o usuário tem permissão para acessar este modelo
        if ($modelo['certificado']->getIdUsuario() != $usuario['usuario']->getId()) {
            echo json_encode([
                'status' => false,
        ...
        // Retornar os dados do modelo em formato JSON
        echo json_encode([
            'status' => true,
...
public function atualizarModeloCertificado(): void
{
    ...
    try {
        ...
        if (!isset($data['id']) || !isset($data['nome']) || !isset($data['orientacao']) || 
        ...
        // Decripta o ID do modelo
        $idDecriptado = Helpers::decryptData($data['id']);
        
        // Primeiro busca o modelo atual para verificar permissões
        $certificadoServico = new CertificadoServico();
        ...
        // Verificar se o usuário tem permissão para editar este modelo
        ...
        // Cria o objeto Certificado com os dados atualizados
