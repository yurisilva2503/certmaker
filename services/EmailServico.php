...
class EmailServico
{
    ...
    public static function enviarEmailRedefinicaoSenha(string $email, Usuario $usuario, string $link): array
    {
        try {
            ...
            $mensagem = "
              <div style='max-width:600px; margin:auto; background:#f5f5f5; border:1px solid #ccc; border-radius:8px; overflow:hidden; font-family: Arial, sans-serif;'>
    ...
    public static function enviarEmailCriacaoConta(string $email, string $nome, string $linkConfirmar, string $linkCancelar): array
    {
        try {
            ...
            // Anexa a imagem para usar com CID no HTML
...
public static function enviarEmailSuporte(string $assunto, string $mensagem, string $prioridade, string $tipoContato, Usuario $usuario): array
{
    try {
        ...
        // Adiciona o usuário como reply-to
        $mail->addReplyTo($usuario->getEmail(), $usuario->getNome());

        $mail->isHTML(true);
        
        // Adiciona prefixo de prioridade no assunto
        $prioridadeLabel = '';
        if ($prioridade === 'alta') {
        ...
        // Labels para prioridade e tipo de contato
        $prioridades = [
            ...
            <div style='max-width:600px; margin:auto; background:#f5f5f5; border:1px solid #ccc; border-radius:8px; overflow:hidden; font-family: Arial, sans-serif;'>
                ...
                <div style='padding:20px; background:#fff;'>
                    ...
                    <div style='background:#f8f9fa; padding:15px; border-radius:5px; margin-bottom:20px;'>
                        ...
                        <p style='font-size:18px; color:#333; margin:0;'>{$assunto}</p>
                    </div>
