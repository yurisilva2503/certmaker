...
class CertificadoServico
{
    ...
    // Cria um novo certificado (agora com páginas)
   ...
   public function salvarCertificadoGerado(array $data, int $id_gerador): array
{
    try {
        ...
        // Converte campos personalizados para JSON se for array
...
public function buscarCertificadoPorCodigo(string $codigoVerificacao): ?array
{
    try {
        ...
        if ($certificado) {
            // Converte o JSON de volta para array
    ...
    public function buscarPorId(int $id): array
{
    try {
        ...
        $certificado = new ModeloCertificado(
            ...
            $dados['nome'],
            $dados['orientacao'],
            $dados['elementos'], // Isso será tratado no construtor
            $dados['paginas'],   // Isso será tratado no construtor
            $dados['background'],
            $dados['criado_em'],
            $dados['atualizado_em']
        );
        
        // Forçar sincronização após criação
    ...
    // Adicione estes métodos na classe CertificadoServico
...
public function copiarModelo(int $id_modelo, string $novo_nome, int $id_usuario): array
{
    try {
        ...
        // Verificar se o usuário tem permissão para copiar este modelo
        // (opcional: pode remover esta verificação se quiser permitir copiar modelos de outros usuários)
        if ($modelo_original['id_usuario'] != $id_usuario) {
            // Se quiser permitir copiar modelos de outros, remova este bloco
            return ['status' => false, 'mensagem' => 'Você não tem permissão para copiar este modelo!'];
        }

        // Inserir o novo modelo copiado
        $sql = "INSERT INTO {$this->table} (id_usuario, nome, orientacao, elementos, paginas, background, criado_em, atualizado_em)
                VALUES (:id_usuario, :nome, :orientacao, :elementos, :paginas, :background, :criado_em, :atualizado_em)";
        $stmt = db::getInstancia()->prepare($sql);
        $stmt->execute([
